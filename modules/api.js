...
app.post('/api/login', async (req, res) => {
  try {
    ...
    else if(user.pending==0&&user.is_banned==0) throw 1004;
    else if(user.pending==0&&user.is_banned==1) throw 1005;
    else if(user.pending==1&&user.is_banned==1) throw 1006;
    else {
...
app.post('/api/sign_up', async (req, res) => {
  try {
    ...
    if(req.body.realname==='') throw 2021;
    if(req.body.idnumber==='') throw 2022;
    if(!/^\d+$/.test(req.body.idnumber)) throw 2023;
...
app.get('/api/sign_up_confirm', async (req, res) => {
  try {
    ...
    if (obj.realname === '') throw new ErrorMessage('真实姓名不能为空。');
    if (obj.idnumber === '') throw new ErrorMessage('学号不能为空。');
