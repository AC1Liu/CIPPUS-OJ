...
export default class ContestRanklist extends Model {
  ...
  async updatePlayer(contest, player, flag) {
    ...
    if (contest.type === 'noi' || contest.type === 'ioi') {
      ...
      {
        for (let player of players) {
          player.maskedtimeSum = 0;
          for (let i in player.score_details) {
            if (player.score_details[i].maskedaccepted) {
              player.maskedtimeSum += (player.score_details[i].maskedacceptedTime - contest.start_time) + (player.score_details[i].maskedunacceptedCount * 20 * 60);
            }
          }
        }

        players.sort((a, b) => {
          if (a.maskedscore > b.maskedscore) return -1;
          if (b.maskedscore > a.maskedscore) return 1;
          if (a.maskedtimeSum < b.maskedtimeSum) return -1;
          if (a.maskedtimeSum > b.maskedtimeSum) return 1;
          return 0;
        });
      }
