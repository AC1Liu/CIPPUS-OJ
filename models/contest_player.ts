      ...
      if (!judge_state.pending) {
        if (!this.score_details[judge_state.problem_id]) {
          this.score_details[judge_state.problem_id] = {
            ...
            unacceptedCount: 0,
            maskedunacceptedCount: 0,
            maskedCount: 0,
            acceptedTime: 0,
            maskedacceptedTime: 0,
            firstTriedTime: 0,
            judge_id: 0,
            maskedjudge_id: 0,
            submissions: {}
        ...
        this.score_details[judge_state.problem_id].maskedunacceptedCount = 0;
        this.score_details[judge_state.problem_id].maskedjudge_id = 0;
        this.score_details[judge_state.problem_id].maskedaccepted = 0;
        this.score_details[judge_state.problem_id].maskedCount = 0;
        for (let x of arr) {
          if(this.score_details[judge_state.problem_id].firstTriedTime===0)
            this.score_details[judge_state.problem_id].firstTriedTime=x.time;
          if (x.accepted) {
            if(x.time<this.contest.rank_stop_time) this.score_details[judge_state.problem_id].maskedaccepted = true;
            if(x.time<this.contest.rank_stop_time) this.score_details[judge_state.problem_id].maskedacceptedTime = x.time;
            ...
            if(x.time<this.contest.rank_stop_time) this.score_details[judge_state.problem_id].maskedunacceptedCount++;
            this.score_details[judge_state.problem_id].unacceptedCount++;
          }
        }

        for (let x of arr) {
          if(x.time>=this.contest.rank_stop_time) this.score_details[judge_state.problem_id].maskedCount++;
          else this.score_details[judge_state.problem_id].maskedjudge_id=x.judge_id;
        }
        ...
        for (let x in this.score_details) {
          ...
          if (this.score_details[x].maskedaccepted) this.maskedscore++;
        }
      }
    }
  }
